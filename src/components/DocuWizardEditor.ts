...
export default defineComponent({
  ...
  setup(props, ctx) {
    const tiny = useTiny()
    const content = tiny.content.html
    const editorInit = ref({
      ...
      toolbar2: 'formatpainter | forecolor backcolor emoticons | help',
      setup: function (editor) {
        ...
        editor.ui.registry.addToggleMenuItem('toggleitem', {
          text: 'Auto save content',
          onAction: function () {
            toggleState = !toggleState
            if (toggleState) {
              tiny.autoSaveContent()
            } else {
              tiny.unAutoSaveContent()
            }
          },
